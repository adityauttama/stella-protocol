...
2. If no ideation was done, suggest it: "Belum ada Idea Brief — jalankan `stella-protocol` untuk brainstorm dulu sebelum define." / "No Idea Brief yet — run `stella-protocol` to brainstorm before defining."
...
**Express → Guided (Deepening):**
At any point during or after Express, Stella can say "pandu saya untuk bagian ini" / "walk me through this section" or "mau lebih detail" / "want to go deeper." Rules:
- Switch to Guided mode for the specific lens Stella wants to deepen
- Already-discussed Express topics become pre-filled context for that lens — show them as the starting point
- Stella can approve the Express version as-is or enrich it through the Guided lens
- After the deepened lens is approved, return to Express for remaining topics
- PRD metadata: `Express + Guided (Lens N deepened)`

**Guided → Express (Accelerating):**
At any point during Guided, Stella can say "generate aja sisanya" / "just generate the rest." Rules:
...
- PRD metadata: `Guided (partial N/7) + Express`
...
**Escape Hatch:** See **Mode Switch** section above for switching between Express and Guided in either direction.
...
## EXIT GATE — MANDATORY before leaving DEFINE

Before suggesting BUILD or any next phase, you MUST output this checklist:

```
EXIT GATE — stella-define
[ ] brain/prd-[name].md exists as a file (not just in conversation)
[ ] brain/architecture.md updated with key decisions
[ ] brain/log-pose.md reflects current phase and state
...
"DEFINE complete. Selanjutnya: jalankan `stella-build` untuk mulai implementasi." / "DEFINE complete. Next: run `stella-build` to start implementation."
